...
def clean_df(df_raw):
    ...
    if df.shape[0] > 0 and df.iloc[0].astype(str).str.contains("unit", case=False, na=False).any():
...
# — drop any duplicate column names (keeps first) —
...
# 7) Insert clickable link column at front
ranked.insert(
    0,
    "🔗",
...
st.write(f"## Top {len(disp)} ({filter_mode})")
st.dataframe(
    disp,
    use_container_width=True,
    height=600,
    column_config={
        "🔗": st.column_config.LinkColumn("Open in Spotify")
    }
)
